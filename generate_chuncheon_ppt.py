# 춘천교육청 코드맵AI 점검 결과 보고서 PPT 생성 (Firebase 34명 전체 데이터)
import json
...
with open("춘천교육청_분석결과.json", "r", encoding="utf-8") as f:
    data = json.load(f)
...
pa = data["posture_analysis"]; na = data["national_avg"]; indiv = data["individual_summary"]
...
box(s, Inches(1.2), Inches(2.7), Inches(11), Inches(1.0), "춘천교육청 3Body 7Code 건강 점검 결과 보고서", 34, True, C_WHITE)
box(s, Inches(1.2), Inches(3.8), Inches(11), Inches(0.5), "AI 기반 신체 . 뇌 . 마음 통합 건강 분석 리포트", 16, False, C_SECONDARY)
for i, line in enumerate([f"점검 일자     2026년 6월 24일", f"점검 인원     {total}명 (남성 {dm['male_count']}명 / 여성 {dm['female_count']}명)", "대상 기관     강원특별자치도 춘천교육청", "점검 방식     코드맵AI 3Body 7Code (LITE 버전)"]):
...
kpi(s, Inches(10.2), Inches(1.5), Inches(2.6), Inches(0.85), "점검일", "2026.06.24", "춘천교육청", C_WHITE, C_ACCENT)
...
box(s, Inches(0.9), Inches(4.4), Inches(11.6), Inches(2.7),
    "코드맵AI 3Body 7Code 점검 시스템은 AI 기반으로 신체(Body), 뇌(Brain), 마음(Mind)의 3가지 영역을 실시간으로 측정하여 개인별 건강 프로파일을 생성하는 통합 건강관리 솔루션입니다.\n\n"
    "- 신체(Body) 분석: TensorFlow.js 기반 실시간 자세 스캔 + AI 체형 정렬 분석\n"
    "- 뇌(Brain) 분석: 시각 반응 속도, 패턴 인지, 기억력 테스트 기반 인지 기능 측정\n"
    "- 마음(Mind) 분석: 설문 + 생체 반응 융합 분석을 통한 정서적 건강 상태 추정\n"
    "- 7Code 에너지: 7가지 에너지 코드로 전신 웰니스 상태를 코드화\n\n"
    "본 보고서는 춘천교육청 소속 34명의 전체 데이터를 Firebase 서버에서 직접 추출하여 분석하고, 전국 3,267명의 평균 데이터와 비교합니다.", 10.5, False, C_TEXT)

# ====== SLIDE 3: 3Body 점수 ======
s = prs.slides.add_slide(blank); bg(s, C_LIGHT); header(s, "3Body 통합 건강 점수 분석", "3BODY ANALYSIS")
labels_3b = [("신체 (Body)", "body", C_INDIGO), ("뇌 (Brain)", "brain", C_ACCENT), ("마음 (Mind)", "mind", C_FUCHSIA)]
for i, (label, key, color) in enumerate(labels_3b):
    v = tb["overall"][key]; n = na["three_body"][key]
...
shdr(tbl, 4)
for r, (label, vals) in enumerate([("춘천 전체", tb["overall"]), ("남성", tb["male"]), ("여성", tb["female"])], 1):
...
shdr(tbl2, 4)
fmt(tbl2.cell(1, 0), "춘천교육청", 10, True, C_TEXT)
fmt(tbl2.cell(2, 0), "전국 평균", 10, True, C_MUTED)
for ci, k in enumerate(["body","brain","mind"], 1):
    v = tb["overall"][k]; n = na["three_body"][k]; d = round(v-n,1)
    color = C_SUCCESS if d>0 else C_DANGER
...
# ====== SLIDE 4: 건강나이 (마음나이 포함) ======
s = prs.slides.add_slide(blank); bg(s, C_LIGHT); header(s, "건강나이 (웰니스 에이지) 분석", "WELLNESS AGE")
age_items = [("실제 나이", hao["avg_real_age"], "세", C_TEXT), ("신체 나이", hao["avg_physical_age"], "세", C_DANGER),
    ("뇌 나이", hao["avg_brain_age"], "세", C_SUCCESS), ("마음 나이", hao["avg_mind_age"], "세", C_FUCHSIA),
    ("종합 나이", hao["avg_comprehensive_age"], "세", C_INDIGO), ("종합 점수", hao["avg_overall_score"], "점", C_ACCENT)]
for i, (label, val, unit, color) in enumerate(age_items):
    sub = ""
    if label in ["신체 나이","뇌 나이","마음 나이"] and val and hao["avg_real_age"]:
        gap = round(val - hao["avg_real_age"], 1)
        sub = f"실제 대비 {'+' if gap>0 else ''}{gap}세"
...
for r, (label, d) in enumerate([("남성", ha["male"]), ("여성", ha["female"])], 1):
    fmt(tbl.cell(r,0),label,10,True,C_TEXT)
    flds = ["avg_real_age","avg_physical_age","avg_brain_age","avg_mind_age","avg_comprehensive_age","avg_overall_score"]
    units = ["세","세","세","세","세","점"]
    for ci, (fld, u) in enumerate(zip(flds, units), 1):
        v = d.get(fld); fmt(tbl.cell(r,ci), f"{v}{u}" if v else "-", 10, False, C_TEXT)
# 인사이트
card(s, Inches(0.6), Inches(4.2), Inches(12.2), Inches(1.2))
mind_gap = round(hao["avg_mind_age"] - hao["avg_real_age"], 1) if hao["avg_mind_age"] and hao["avg_real_age"] else 0
brain_gap = round(hao["avg_brain_age"] - hao["avg_real_age"], 1) if hao["avg_brain_age"] and hao["avg_real_age"] else 0
phys_gap = round(hao["avg_physical_age"] - hao["avg_real_age"], 1) if hao["avg_physical_age"] and hao["avg_real_age"] else 0
box(s, Inches(0.9), Inches(4.35), Inches(11.6), Inches(1.0),
    f"핵심 인사이트:  실제 나이 {hao['avg_real_age']}세 대비, 신체 나이 +{abs(phys_gap)}세 높고, "
    f"뇌 나이는 {abs(brain_gap)}세 {'젊으며' if brain_gap<0 else '높고'}, "
    f"마음 나이는 {abs(mind_gap)}세 {'높습니다' if mind_gap>0 else '젊습니다'}. "
    f"마음 나이가 가장 빠르게 노화되고 있어, 스트레스 관리와 이완 훈련이 시급합니다.", 11, False, C_TEXT)
...
shdr(tbl, 8)
for r, (rl, rd) in enumerate([("춘천교육청", sc["overall"]), ("전국 평균", na["seven_code"])], 1):
...
for i in range(7):
    v=sc["overall"][f"c{i+1}"]; n=na["seven_code"][f"c{i+1}"]; d=round(v-n,1)
    color = C_SUCCESS if d>0 else C_DANGER
...
for i, (cn, info) in enumerate(sorted_wc[:3]):
    x = Inches(0.6 + i * 4.2)
    rc = [C_DANGER, C_ACCENT, C_SECONDARY][i]
...
top_wc = sorted_wc[0] if sorted_wc else None
box(s, Inches(0.9), Inches(5.85), Inches(11.6), Inches(1.2),
    f"핵심 인사이트:  춘천교육청 공무원의 {top_wc[1]['ratio']}%가 코드{top_wc[0]}({top_wc[1]['label']}) 영역에서 가장 취약합니다. "
    f"이는 전국 평균({na['weakest_code_ratio'].get(int(top_wc[0]),0)}%)의 2배 이상으로, "
    f"심신 이완 능력의 현저한 결핍을 보여줍니다. "
    f"반면 코드5(소통), 코드6(집중통찰), 코드7(삶의방향)은 전국 대비 +5~7점 높아, "
    f"업무 관련 인지 능력과 소통 역량은 우수합니다." if top_wc else "", 10.5, False, C_TEXT)
...
if pa:
    ...
    shdr(tbl, 5)
    for r, name in enumerate(pk, 1):
        st = pa[name]
        fmt(tbl.cell(r,0),name,9,False,C_TEXT,PP_ALIGN.LEFT); fmt(tbl.cell(r,1),f"{st['Good']}%",10,False,C_SUCCESS)
    ...
    for i, h in enumerate(["지표","춘천 Poor","전국 Poor","비교"]): fmt(tbl2.cell(0,i),h,10,True,C_WHITE)
    shdr(tbl2, 4)
    for r, nn in enumerate(compare, 1):
        ...
        nv = nat_p[nn]["Poor"]
        fmt(tbl2.cell(r,0),nn,8,False,C_TEXT,PP_ALIGN.LEFT)
        fmt(tbl2.cell(r,1),f"{cc_val}%" if cc_val is not None else "-",10,True,C_TEXT)
...
tbo = tb["overall"]
findings = [
    (f"마음 나이가 실제보다 {abs(mind_gap)}세 높습니다",
     f"평균 실제 나이 {hao['avg_real_age']}세 대비 마음 나이 {hao['avg_mind_age']}세로, "
     f"정서적 노화가 가장 빠릅니다. 마음 점수 {tbo['mind']}점은 전국 평균과 유사하며, "
     f"이완·명상 프로그램 도입이 시급합니다.", C_FUCHSIA),
    (f"코드1(이완 능력) {sorted_wc[0][1]['ratio']}%가 최취약 - 전국의 2배 이상",
     f"34명 중 {sorted_wc[0][1]['count']}명이 이완 능력에서 최하위 점수를 기록했습니다. "
     f"자율신경계 과긴장 상태가 만연해 있으며, 호흡법·명상 훈련이 필요합니다.", C_DANGER),
    ("뇌 건강 양호, 인지 역량 우수",
     f"뇌 점수 {tbo['brain']}점(전국 대비 +{round(tbo['brain']-na['three_body']['brain'],1)}), "
     f"뇌 나이 {hao['avg_brain_age']}세(실제 대비 {brain_gap}세). "
     f"코드5·6·7도 전국 대비 +5~7점으로 업무 관련 인지 역량이 우수합니다.", C_SUCCESS),
    ("신체 나이 +4.2세, 근골격계 관리 필요",
     f"신체 나이 {hao['avg_physical_age']}세로 실제 대비 {phys_gap}세 높습니다. "
     f"장시간 좌식 근무로 인한 자세 불량을 개선하기 위한 체형 교정 프로그램을 권장합니다.", C_ACCENT),
    ("정기 점검 및 조직 건강 프로그램 도입 제안",
     "6개월 간격의 코드맵AI 정기 점검으로 건강 변화를 추적하고, "
     "조직 차원의 이완 훈련, 점심 스트레칭, 뇌 활성화 훈련 도입을 제안합니다.", C_SECONDARY),
]
for i, (title, desc, color) in enumerate(findings):
    y = Inches(1.5 + i * 1.1)
    c = card(s, Inches(0.6), y, Inches(12.2), Inches(0.95), RGBColor(30, 41, 59))
    c.line.color.rgb = RGBColor(51, 65, 85)
    bar = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.6), y, Inches(0.15), Inches(0.95))
    bar.fill.solid(); bar.fill.fore_color.rgb = color; bar.line.fill.background()
    box(s, Inches(1.0), y+Inches(0.08), Inches(11.5), Inches(0.3), title, 13, True, C_WHITE)
    box(s, Inches(1.0), y+Inches(0.4), Inches(11.5), Inches(0.5), desc, 10, False, C_SLATE)
...
output = "춘천교육청_코드맵AI_결과보고서.pptx"
prs.save(output)
print(f"PPT 생성 완료: {output} ({len(prs.slides)}개 슬라이드)")
