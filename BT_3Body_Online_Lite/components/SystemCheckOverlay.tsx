    ...
    <div className="absolute inset-0 z-[100] bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center p-6">
      {phase === 'pc' && (
        <div className="bg-slate-800 p-10 rounded-[2.5rem] border border-slate-700 shadow-[0_20px_50px_rgba(0,0,0,0.5)] max-w-md w-full text-center animate-fade-in">
          ...
          <h3 className="text-2xl font-black text-white mb-8 tracking-tight">{t("PC 사양 점검")}</h3>
          
          <div className="space-y-4 text-left mb-10">
            <div className="flex justify-between items-center bg-slate-900/50 p-5 rounded-2xl border border-slate-700/50">
              <span className="text-slate-400 font-medium">{t("CPU 코어 수")}</span>
            ...
            <div className="flex justify-between items-center bg-slate-900/50 p-5 rounded-2xl border border-slate-700/50">
              <span className="text-slate-400 font-medium">{t("메모리 (RAM)")}</span>
              <span className="text-white font-bold">{isCheckingPc ? <div className="w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div> : `${ramSize} GB ${t("이상")}`}</span>
            </div>
          ...
          >
            {t("다음 단계 (카메라 및 조명 체크)")} <i className="fas fa-arrow-right ml-2 text-xs"></i>
          </button>
        </div>
      )}
      ...
      {phase === 'camera' && (
        <div className="w-full max-w-3xl bg-slate-800 p-8 rounded-[2.5rem] border border-slate-700 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col items-center animate-fade-in relative overflow-hidden">
          <div className="text-center mb-6 z-10">
            <span className="text-indigo-400 font-bold text-xs tracking-widest mb-2 block">{t("환경 체크 2단계")}</span>
            <h3 className="text-3xl font-black text-white tracking-tight">{t("전신 카메라 점검")}</h3>
            <p className="text-slate-400 mt-3 text-sm leading-relaxed max-w-lg mx-auto">
              {t("화면에 머리부터 발끝까지 전신이 들어오는지 확인합니다.")}<br/>
              {t("혼자 계실 경우 중앙의 5초 후 자동 촬영 버튼을 눌러")}<br/>
              {t("시간 내에 전신이 나오는 위치로 이동하여 테스트를 마쳐주세요.")}
            </p>
          </div>
      ...
      {phase === 'done' && (
        <div className="bg-slate-800 p-12 rounded-[2.5rem] border border-emerald-500/40 shadow-[0_20px_50px_rgba(16,185,129,0.2)] max-w-md w-full text-center animate-fade-in relative overflow-hidden">
          ...
          <h3 className="text-3xl font-black text-white mb-3">{t("점검 완료!")}</h3>
          <p className="text-emerald-400 font-medium">
            {t("모든 진단 환경이 준비되었습니다.")}<br/>
            {t("메인 시스템으로 이동합니다.")}
          </p>
        </div>
      )}
