...
def run(input_file, institution, test_date, output_file):
    with open(input_file, "r", encoding="utf-8") as f:
        raw = json.load(f)

    print(f"raw 데이터: {len(raw)}건")

    # 중복 제거 (이름 기준, report + threeBodyAnalysis 있는 것 우선)
    dedup = {}
    for m in raw:
        report = m.get("report", {})
    ...
    for m in members:
        ...
        for pm in (pm_list or []):
            if isinstance(pm, dict):
                posture_metrics.append({"name": pm.get("name", ""), "status": pm.get("status", ""), "score": sf(pm.get("score"))})
    ...
    total = len(parsed)
    if total == 0:
        print("오류: 분석 대상 데이터가 없습니다.", file=sys.stderr)
        sys.exit(1)
    ...
    ag_order = ["10대이하","20대","30대","40대","50대","60대","70대","80대이상"]
    demographics = {
        ...
        "age_groups": dict(sorted(age_groups_dist.items(),
            key=lambda x: ag_order.index(x[0]) if x[0] in ag_order else 99)),
    }

    # 3Body
    def tb_stats(group):
        return {"body": savg([p["body_score"] for p in group]), "brain": savg([p["brain_score"] for p in group]), "mind": savg([p["mind_score"] for p in group])}
    three_body = {"overall": tb_stats(parsed), "male": tb_stats(males), "female": tb_stats(females)}
    three_body_by_age = {}
    for ag_name in ag_order:
        grp = [p for p in parsed if p["age_group"] == ag_name]
        if grp: three_body_by_age[ag_name] = {"count": len(grp), **tb_stats(grp)}

    # 건강나이
    def ha_stats(group):
        return {
            "avg_real_age": savg([p["age"] for p in group]),
            "avg_physical_age": savg([p["physical_age"] for p in group]),
            "avg_brain_age": savg([p["brain_age"] for p in group]),
            "avg_mind_age": savg([p["mind_age"] for p in group]),
            "avg_face_age": savg([p["face_age"] for p in group]),
            "avg_comprehensive_age": savg([p["comprehensive_age"] for p in group]),
            "avg_overall_score": savg([p["overall_score"] for p in group]),
        }
    health_age = {"overall": ha_stats(parsed), "male": ha_stats(males), "female": ha_stats(females)}
    ...
    result = {
        "event_code": raw[0].get("eventCode", "") if raw else "",
    ...
    # 콘솔 요약
    tb = three_body["overall"]; hao = health_age["overall"]
    print(f"\n[인구통계] {total}명 (남 {len(males)}, 여 {len(females)}), 평균 {demographics['avg_age']}세")
    print(f"[3Body] 신체={tb['body']} | 뇌={tb['brain']} | 마음={tb['mind']}")
    print(f"[건강나이] 실제={hao['avg_real_age']} | 신체={hao['avg_physical_age']} | 뇌={hao['avg_brain_age']} | 마음={hao['avg_mind_age']}")
    print(f"[종합] 종합나이={hao['avg_comprehensive_age']} | 점수={hao['avg_overall_score']}점")
    print(f"\nSuccess! Data written to: {output_file}")

def main():
    parser = argparse.ArgumentParser(description="BTC 단체 건강 점검 데이터 통계 분석")
    parser.add_argument("--input", required=True, help="Firebase raw JSON 파일 경로")
    parser.add_argument("--institution", required=True, help="기관명 (예: 춘천교육청)")
    parser.add_argument("--date", required=True, help="점검일 (예: 2026-06-24)")
    parser.add_argument("--output", required=True, help="분석 결과 JSON 출력 경로")
    args = parser.parse_args()
    run(args.input, args.institution, args.date, args.output)
