 ...
 *
 * POST /api/admin-errorlog
 * Body: { action: 'list'|'updateStatus', id?, status? }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === 'list') return res.status(200).json({ logs: await listErrorLogs() });
    if (action === 'updateStatus') {
      if (status !== 'new' && status !== 'viewed' && status !== 'resolved') return res.status(400).json({ error: 'invalid status' });
      if (!id) return res.status(400).json({ error: 'id 필요' });
      await setErrorLogStatus(id, status);
      return res.status(200).json({ ok: true });
    }
    return res.status(400).json({ error: 'unknown action' });
  } catch (e: any) {
    console.error('[admin-errorlog] Server Error:', e);
